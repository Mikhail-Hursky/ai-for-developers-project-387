...
- [ ] **Step 4: Прогнать весь набор дважды подряд**

Run: `npm test && npm test`
Expected: оба раза 4 passed. Второй прогон подтверждает, что тесты переживают непустое хранилище — брони первого прогона остаются в памяти сервера, если он переиспользован.
