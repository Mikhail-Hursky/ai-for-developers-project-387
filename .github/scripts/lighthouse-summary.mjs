...
const summary = [
  ...
  'Проваленные аудиты:',
  // Сюда попадаем, только если ни на одной странице ни в одной категории не
  // нашлось ни одного аудита со score < 1 — то есть каждая категория и так
  // набрала 100 (иначе median.report.categories[id].score был бы ниже 1, и
  // failures.length был бы больше нуля). "Проблемных категорий" здесь уже нет:
  // формулировка должна быть верна и когда ниже порога никого нет.
  ...(auditLines.length > 0 ? auditLines : ['- проваленных аудитов не найдено: везде score 1.']),
  '',
